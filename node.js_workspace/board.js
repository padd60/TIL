...
// app.use('.js', express.static('js'))//js폴더 사용
// app.use('./img', express.static('img'))//img폴더 사용
...
app.get("/insert", function (request, response) {
  // 파일을 읽습니다.
  fs.readFile("views/board_insert.html", "utf8", function (error, data) {
    // 응답합니다.
    response.send(data);
  });
});
...
app.get("/content/:id", function (request, response) {
  // 파일을 읽습니다.
  fs.readFile("views/board_content.html", "utf8", function (error, data) {
    // 데이터베이스 쿼리를 실행합니다.
    client.query(
...
app.post("/edit/:id", function (request, response) {
  ...
  client.query(
    ...
    function () {
      // 응답합니다.
      // response.redirect("/"); // 목록으로 이동
      response.redirect("/content/" + request.params.id); //상세보기로 이동
    }
  );
});
//글삭제
